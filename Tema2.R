...
matriz <- rbind(vector_1, vector_2, vector_3, vector_4) #row bind

matriz


matriz2 <- cbind(vector_1, vector_2, vector_3, vector_4) #column bind 
matriz2
class(matriz2)
...
matriz4[matriz4>30]
matriz4[matriz4 !=14 & matriz4 <= 26]

#regresa valor de indices y retorna un array con los valores que cumplen la condicion
which(matriz4 !=14 & matriz4 <= 26)

?which

#string es mandante. Es mas facil convertir string a num 
x <-c("a", 34, "c", 34)
x
...
mi_df <- data.frame(
  ...
  "intento_matriz" = matrix(1:4, ncol = 2, nrow = 2)
)
...
dim(murders) #regresa total de observarciones y columnas 
...
filter(new_table, total<=2) #filtra el campo total menor o igual a 300
